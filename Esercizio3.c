...
int main()
{
    ...
    while ((carattere=getchar())!='#'){ //CICLO PER OGNI CARATTERE DEL TESTO

           if (isupper(carattere)){ //CARATTERE MAIUSCOLO
                ++nMaiuscole;
           }if (islower(carattere)){ //CARATTERE MINUSCOLO
               ++nMinuscole;
           }if (isspace(carattere)){ //SPAZIO
               ++nSpazi;
           ...
           ++lTesto; //AGGIUNGO UNO ALLA LUNGHEZZA TOTALE DEL TESTO COMPRESO '#'
    }
    ...
    printf("\n\nLETTERE MAIUSCOLE: %u\n"
           ...
           "LUNGHEZZA TOTALE DEL TESTO(compreso '#'): %u",nMaiuscole,nMinuscole,nSpazi,nPunteggiatura,nNumeri,nGrafici,nStamp,nControl,
                                            nAlNum,nAlpha,nBlack,lTesto);
